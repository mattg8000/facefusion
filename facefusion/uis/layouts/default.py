...
from facefusion.uis.components import about, age_modifier_options, background_remover_options, common_options, deep_swapper_options, download, execution, execution_thread_count, expression_restorer_options, face_database, face_debugger_options, face_detector, face_editor_options, face_enhancer_options, face_landmarker, face_mapping, face_masker, face_selector, face_swapper_options, frame_colorizer_options, frame_enhancer_options, instant_runner, job_manager, job_runner, lip_syncer_options, memory, output, output_options, preview, preview_options, processors, source, target, temp_frame, terminal, trim_frame, ui_workflow, voice_extractor, forced_replacement
	...
	with gradio.Blocks() as layout:
		with gradio.Row():
			...
			with gradio.Column(scale = 7):
				...
				with gradio.Blocks():
					face_database.render()
				with gradio.Blocks():
					face_mapping.render()
				with gradio.Blocks():
					forced_replacement.render()
				with gradio.Blocks():
...
def listen() -> None:
	...
	face_database.listen()
	face_mapping.listen()
	forced_replacement.listen()
